...
// סינון כרטיסים זמינים בלבד (status === 1)
const availableTickets = allTickets.filter((t) => t.status === 0 && !t.isExternal);

// יצירת רשימת אירועים ייחודיים מתוך כרטיסים זמינים בלבד
const uniqueEvents = Array.from(
  new Map(availableTickets.map((t) => [t.event.id, t.event])).values()
);

// סינון כרטיסים לאירוע שנבחר
const ticketsForSelectedEvent = availableTickets.filter(
  (t) => t.event.id === selectedEvent?.id
);
    ...
    {showOptions && (
      <>
        ...
        <View style={styles.optionsOverlay}>
          ...
          <Text style={styles.optionText}>Selling ticket inside PockTick</Text>
...
{step === 1 && (
  <>
    {/* לוגו + טקסט שורת כותרת */}
<View style={styles.headerRow}>
  <Image source={require('../../assets/images/just_logo.png')} style={styles.headerLogo} />
  <Text style={styles.headerText}>Selling ticket inside Pocktick</Text>
</View>
    {/* קו כחול */}
    <View style={styles.headerLine} />

    {/* כותרת הקטגוריה */}
    <Text style={styles.sectionTitle}>Select Event to Transfer</Text>
      ...
      {uniqueEvents.map((event) => {
        ...
        return (
          ...
          >
            ...
            <Image source={require('../../assets/icons/chevron.png')} style={{ width: 20, height: 20 }} />
          </TouchableOpacity>
        );
      })}
    </ScrollView>
      ...
      {ticketsForSelectedEvent.map((ticket) => {
        ...
        return (
          ...
          >
            ...
            {/* עיגול צבעוני עם תמונה קבועה */}
            <View
              style={[
                styles.circleIconWrapper,
                { backgroundColor: isSelected ? '#FDB813' : '#ccc' }, // שינוי צבע רקע לפי בחירה
              ]}
            >
              <Image
...
<View style={[styles.inputWrapper, { alignItems: 'flex-start' }]}>
  ...
  />
    ...
    >
      <View style={{ width: 30 }} />
      <Text style={styles.continueText}>Confirm Transfer</Text>
      <View style={styles.iconCircle}>
